...
def is_sub_model_grant(model_grant, parent_model_grant):
    return all(
...
def is_sub_permissions_grant(permissions_grant, parent_permissions_grant):
    return set(permissions_grant).issubset(parent_permissions_grant)


def is_sub_grant(grant, parent_grant):
    model_grant, permissions_grant = grant
    parent_model_grant, parent_permissions_grant = parent_grant
    return is_sub_model_grant(model_grant, parent_model_grant) and is_sub_permissions_grant(permissions_grant, parent_permissions_grant)


def is_sub_scope(scope, parent_scope):
    # Check each grant of the scope are allowed.
    return all(
        any(
            is_sub_grant(grant, parent_grant)
            for parent_grant
            in parent_scope
        )
        for grant
